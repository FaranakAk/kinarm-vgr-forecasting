...
# ---------------------------------------------------------------------------
#  Added:
#   • a small ChronosDataset in "validation" mode (10 % of each Arrow shard)
#   • Hugging Face EarlyStoppingCallback (patience + threshold CLI flags)
#   • evaluation_strategy="steps" and eval_steps
#   • --early-stopping-patience and --early-stopping-threshold CLI options
# ---------------------------------------------------------------------------
...
# os.environ["TQDM_DISABLE"] = "0"          # make sure tqdm is on
# os.environ["TQDM_NOTEBOOK"] = "0"         # force std bar, not HTML

# os.environ["ACCELERATE_DISABLE_TORCH_COMPILE"] = "1"
os.environ["TQDM_FRONTEND"] = "cli"    # ← plain ASCII bar
# os.environ["DISABLE_TQDM"] = "1"     # (alternative: turn bar off)
import re
import sys
import json
import itertools
...
def main(
    ...
    save_total_limit: int = 3,          # ← new line
    log_steps: int = 500,
    eval_steps: int = 2_000,                       # NEW default
    ...
    early_stopping_patience: int = 5,             # NEW CLI flag
    early_stopping_threshold: float = 0.0,        # NEW CLI flag
    ...
    # TRAIN dataset (shuffled iterable)
    train_iter = ChronosDataset(
        datasets=train_datasets,
        probabilities=probability,
        tokenizer=tokenizer,
        context_length=context_length,
    ...
    # VALIDATION dataset – one deterministic window per series
    val_iter = ChronosDataset(
        datasets=train_datasets,
        ...
        model_type=model_type,
        imputation_method=LastValueImputation() if model_type == "causal" else None,
        mode="validation",           # <-- sampler picks the last viable window
    )
    ...
    training_args = TrainingArguments(
        ...
        save_total_limit=save_total_limit,  # ← add here
        evaluation_strategy="steps",      # NEW
        eval_steps=eval_steps,             # NEW
    ...
    # print(">>> torch_compile in TrainingArguments =", training_args.torch_compile)
    # training_args.disable_tqdm = False         # show tqdm
    # os.environ["TOKENIZERS_PARALLELISM"] = "false"   # tidy warning
    
    # # and force plain bar
    # from transformers.utils import logging as hf_logging
    # hf_logging.set_verbosity_info()            # enable info level
    ...
    trainer = Trainer(
        ...
        eval_dataset=val_iter,                  # NEW
        callbacks=[early_stop_cb],              # NEW
    )
    log_on_main("Training with early stopping", logger)

    trainer.train()

    if is_main_process():
        model.save_pretrained(output_dir / "checkpoint-final")
        save_training_info(output_dir / "checkpoint-final", raw_training_config)
