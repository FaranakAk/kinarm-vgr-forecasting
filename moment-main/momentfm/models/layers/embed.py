...
class PatchEmbedding(nn.Module):
    def __init__(
        ...
        value_embedding_bias: bool = False,
        orth_gain: float = 1.41,
        num_directions: int = 8,  # Add number of possible directions
    ):
        super(PatchEmbedding, self).__init__()
        self.patch_len = patch_len
        self.seq_len = seq_len
        ...
        # Direction embedding (new)
        self.direction_embedding = nn.Embedding(num_directions, d_model)

        # Residual dropout
        self.dropout = nn.Dropout(dropout)

    def forward(self, x: torch.Tensor, mask: torch.Tensor = None, directions: torch.Tensor = None) -> torch.Tensor:
        """
        Args:
            x: Tensor of shape [batch_size, n_channels, seq_len]
            mask: Tensor of shape [batch_size, seq_len]
            directions: Tensor of shape [batch_size, n_patches], containing direction indices for each patch
        Returns:
            Embedded tensor of shape [batch_size, n_channels, n_patches, d_model]
        """
        # Convert mask to patch view
        mask = Masking.convert_seq_to_patch_view(
            mask, patch_len=self.patch_len
        ).unsqueeze(-1)
        # mask : [batch_size x n_patches x 1]
        ...
        #     direction_embeds = self.direction_embedding(directions)  # [batch_size, n_patches, d_model]
        #     direction_embeds = direction_embeds.unsqueeze(1).repeat(1, n_channels, 1, 1)
        #     x = x + direction_embeds
